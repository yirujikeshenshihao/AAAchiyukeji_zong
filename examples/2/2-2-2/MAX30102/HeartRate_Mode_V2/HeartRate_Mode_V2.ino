...
#define heartTime 10  //检测心率时间
unsigned long heartLastTime = 0;
...
int beatAvg;               //平均心率
bool heartFlag = false;    //检测心率状态
/*信息打印*/
#define printTime 1000;
unsigned long printLastTime = 0;  //存储时间戳
void setup(void) {
...
void loop(void) {
  ...
  getBmp();                                //获取心率状态，放在loop中，一直获取数据
  if (millis() > printLastTime) {          //判断是否到达串口打印时间
    printLastTime = millis() + printTime;  //更新时间
    if (heartFlag == true) {
      if (millis() > heartLastTime) {
        Serial.print("BMP:");
...
void getBmp() {
  ...
  if (irValue < 50000) {
    heartFlag = false;  //不在检测心率状态
  } else {

    if (heartFlag == false) {
      heartFlag = true;                             //在检测心率状态
      heartLastTime = millis() + heartTime * 1000;  //更新检测时间
    }
  }
}
